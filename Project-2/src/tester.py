...
def tester_5():
    ...
    # # test LSI
    # for case in testcases:
    #     predicted_labels = kmeans.predict(lsi_docs_tfidf[:, :case])
    #     evaluate.eval_report(ground_truth, predicted_labels, "-----[LSI] r = %d-----" % case)
    #     evaluate.contingency_matrix(ground_truth, predicted_labels, n_clusters=20, 
    #         msg="Contingency Matrix")

    # # test NMF
    # for case in testcases:
        
    #     predicted_labels = kmeans.predict(nmf_docs_tfidf[:, :case])
    #     evaluate.eval_report(ground_truth, predicted_labels, "-----[NMF] r = %d-----" % case)
    #     evaluate.contingency_matrix(ground_truth, predicted_labels, n_clusters=20, 
    #         msg="Contingency Matrix")
    ...
    utils.print_title("Task 5 part(b): Visulize Clustering Result")


    kmeans = kmeans = KMeans(n_clusters=20)

    predicted_labels1 = kmeans.predict(lsi_docs_tfidf)
    predicted_labels2 = kmeans.predict(nmf_docs_tfidf)
    print("-----%s-----" % "LSI with r = %d" % lsi_best_r)
    utils.plot_cluster_result(lsi_docs_tfidf, predicted_labels1, "LSI with r = %d" % lsi_best_r)
    print("-----%s-----" % "NMF with r = %d" % nmf_best_r)
    utils.plot_cluster_result(nmf_docs_tfidf, predicted_labels2, "NMF with r = %d" % nmf_best_r)


    # task 5 part(c)
    utils.print_title("Task 5 part(c): Normalization & non-linear transformation")

    # original result
   
    evaluate.eval_report(ground_truth, predicted_labels1, "-----Original %s-----" % "LSI with r = %d" % lsi_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels1, n_clusters=20, 
        msg="Contingency Matrix")
    
    evaluate.eval_report(ground_truth, predicted_labels2, "-----Original %s-----" % "NMF with r = %d" % lsi_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels2, n_clusters=20, 
        msg="Contingency Matrix")


    # evaluate normalization
    normalized_features1 = utils.normalize(lsi_docs_tfidf)
    predicted_labels1 = KMeans(n_clusters=20).predict(normalized_features1)
    evaluate.eval_report(ground_truth, predicted_labels1, "-----Normalization %s-----" % "LSI with r = %d" % lsi_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels1, n_clusters=20, 
        msg="Contingency Matrix")
    utils.plot_cluster_result(normalized_features1, predicted_labels1, "Normalization %s" % "LSI with r = %d" % lsi_best_r)

    normalized_features2 = utils.normalize(nmf_docs_tfidf)
    predicted_labels2 = KMeans(n_clusters=20).predict(normalized_features2)
    evaluate.eval_report(ground_truth, predicted_labels2, "-----Normalization %s-----" % "NMF with r = %d" % nmf_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels2, n_clusters=20, 
        msg="Contingency Matrix")
    utils.plot_cluster_result(normalized_features2, predicted_labels2, "Normalization %s" % "NMF with r = %d" % nmf_best_r)


    # Non-linear transform (logarithm)
    log_features = utils.log_transform(nmf_docs_tfidf)
    predicted_labels = KMeans(n_clusters=20).predict(log_features)
    evaluate.eval_report(ground_truth, predicted_labels, "-----Logarithm %s-----" % "NMF with r = %d" % nmf_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels, n_clusters=20, 
        msg="Contingency Matrix")
    utils.plot_cluster_result(log_features, predicted_labels, "Logarithm %s" % "NMF with r = %d" % nmf_best_r)

    # Logarithm -> normalization
    log_normalized_features = utils.normalize(log_features)
    predicted_labels = KMeans(n_clusters=20).predict(log_normalized_features)
    evaluate.eval_report(ground_truth, predicted_labels, "-----Logarithm + Normalization %s-----" % "NMF with r = %d" % nmf_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels, n_clusters=20, 
        msg="Contingency Matrix")
    utils.plot_cluster_result(log_normalized_features, predicted_labels, "log_Normalize %s" % "NMF with r = %d" % nmf_best_r)

    # normalization -> logarithm
    normalized_log_features = utils.log_transform(normalized_features2)
    predicted_labels = KMeans(n_clusters=20).predict(normalized_log_features)
    evaluate.eval_report(ground_truth, predicted_labels, "-----Normalization + Logarithm %s-----" % "NMF with r = %d" % nmf_best_r)
    evaluate.contingency_matrix(ground_truth, predicted_labels, n_clusters=20, 
        msg="Contingency Matrix")
